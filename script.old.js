...
function displayRecipes(recipes) {
  ...
  updateFilters(recipes);  // Met à jour les options de filtre en fonction des recettes affichées
}
...
function updateFilterOptions(filterSelector, items) {
...
function filterRecipes() {
  ...
  const appareilSelected = document.querySelector("#appareil-filter").value.toLowerCase();
  const ustensilesSelected = document.querySelector("#ustensiles-filter").value.toLowerCase();
  const ingredientsSelected = document.querySelector("#ingredients-filter").value.toLowerCase();

  const filteredRecipes = recipes.filter(recipe => {
    return (
      (searchValue === "" || recipe.name.toLowerCase().includes(searchValue) || recipe.description.toLowerCase().includes(searchValue) || recipe.ingredients.some(ingredient => ingredient.ingredient.toLowerCase().includes(searchValue))) &&
      (appareilSelected === "" || recipe.appliance.toLowerCase() === appareilSelected) &&
      (ustensilesSelected === "" || recipe.ustensils.some(ustensil => ustensil.toLowerCase().includes(ustensilesSelected))) &&
      (ingredientsSelected === "" || recipe.ingredients.some(ingredient => ingredient.ingredient.toLowerCase().includes(ingredientsSelected)))
    );
  });
...
// Fonction pour filtrer les options du menu déroulant en fonction de l'entrée utilisateur
function filterDropdown(event, filterSelector) {
  const input = event.target.value.toLowerCase();
  const filterElement = document.querySelector(filterSelector);
  const options = filterElement.querySelectorAll("option");
...
function addActiveFilter(filterType, filterValue) {
  ...
  updateActiveFiltersDisplay();  // Met à jour l'affichage des filtres actifs
  filterRecipes();  // Filtre les recettes en fonction des nouveaux filtres
}
...
function updateActiveFiltersDisplay() {
  ...
  Object.keys(activeFilters).forEach(filterType => {
    activeFilters[filterType].forEach(filterValue => {
      ...
      removeButton.addEventListener("click", () => removeActiveFilter(filterType, filterValue));
...
// Event listeners pour le champ de recherche et les filtres
document.querySelector("#main-search").addEventListener("input", filterRecipes);
document.querySelector("#appareil-filter").addEventListener("change", event => {
  const value = event.target.value;
  if (value) {
    addActiveFilter("appareils", value);
  }
});
document.querySelector("#ustensiles-filter").addEventListener("change", event => {
  const value = event.target.value;
  if (value) {
    addActiveFilter("ustensiles", value);
  }
});
document.querySelector("#ingredients-filter").addEventListener("change", event => {
  const value = event.target.value;
  if (value) {
    addActiveFilter("ingredients", value);
  }
});

document.querySelector("#ingredients-filter-label").addEventListener("click", toggleDropdown);
document.querySelector("#appareil-filter-label").addEventListener("click", toggleDropdown);
document.querySelector("#ustensiles-filter-label").addEventListener("click", toggleDropdown);

document.querySelector("#ingredients-filter-input").addEventListener("input", event => filterDropdown(event, "#ingredients-filter"));
document.querySelector("#appareil-filter-input").addEventListener("input", event => filterDropdown(event, "#appareil-filter"));
document.querySelector("#ustensiles-filter-input").addEventListener("input", event => filterDropdown(event, "#ustensiles-filter"));

// Affiche toutes les recettes initialement
...
function linearSearch(array, value) {
  for (let i = 0; i < array.length; i++) {
    if (array[i] === value) {
      console.log(`Linear search found ${value} at index ${i}`);
      return i;
    }
  }
  console.log(`Linear search did not find ${value}`);
  return -1;
}
...
function binarySearch(array, value) {
  ...
  console.log(`Binary search did not find ${value}`);
  return -1;
}

// document.addEventListener('DOMContentLoaded', () => {
//   const ellipsisBlock = document.getElementById('ellipsis-block');
//   const maxHeight = parseInt(getComputedStyle(ellipsisBlock).maxHeight);
  
//   if (ellipsisBlock.scrollHeight > maxHeight) {
//       const afterElement = document.createElement('span');
//       afterElement.textContent = '...';
//       afterElement.style.position = 'absolute';
//       afterElement.style.right = '0';
//       afterElement.style.bottom = '0';
//       afterElement.style.background = 'white'; // Match background color of your block
//       afterElement.style.padding = '0 5px';

//       ellipsisBlock.appendChild(afterElement);
//   }
// });