  ...
  return `
    <div class="recipe-card">
      ...
      <div class="recipe-details">
        <h2>${recipe.name}</h2>
        <div class="recette">
          <p ><strong>Recette:</strong> <br><br> ${recipe.description}</p>
        </div>
        <div class="ingre">
...
function filterRecipes() {
  ...
  const filteredRecipes = recipes.filter(recipe => {
    const matchesSearch = !searchValue || recipe.name.toLowerCase().includes(searchValue) || 
      recipe.description.toLowerCase().includes(searchValue) || 
      recipe.ingredients.some(ingredient => ingredient.ingredient.toLowerCase().includes(searchValue));
    
    const matchesAppareil = !appareilSelected || recipe.appliance.toLowerCase() === appareilSelected;
    
    const matchesUstensiles = !ustensilesSelected || recipe.ustensils.some(ustensil => ustensil.toLowerCase().includes(ustensilesSelected));
    
    const matchesIngredients = !ingredientsSelected || recipe.ingredients.some(ingredient => ingredient.ingredient.toLowerCase().includes(ingredientsSelected));

    return matchesSearch && matchesAppareil && matchesUstensiles && matchesIngredients;
  });
...
document.getElementById('ingredients-filter').addEventListener('change', event => {
  addActiveFilter('ingredients', event.target.value);
});

document.getElementById('appareil-filter').addEventListener('change', event => {
  addActiveFilter('appareils', event.target.value);
});

document.getElementById('ustensiles-filter').addEventListener('change', event => {
  addActiveFilter('ustensiles', event.target.value);
});
...
// Event listeners for the search and filters
document.getElementById("main-search").addEventListener("input", filterRecipes);
document.getElementById("ingredients-filter-label").addEventListener("click", toggleDropdown);
document.getElementById("appareil-filter-label").addEventListener("click", toggleDropdown);
document.getElementById("ustensiles-filter-label").addEventListener("click", toggleDropdown);

document.getElementById("ingredients-filter-input").addEventListener("input", (event) => filterDropdown(event, "ingredients-filter"));
document.getElementById("appareil-filter-input").addEventListener("input", (event) => filterDropdown(event, "appareil-filter"));
document.getElementById("ustensiles-filter-input").addEventListener("input", (event) => filterDropdown(event, "ustensiles-filter"));
